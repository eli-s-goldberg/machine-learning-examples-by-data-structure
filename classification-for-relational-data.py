...
print(train_test_target_data_numpy)
...
for train_index, test_index in repeatKfold.split(train_test_training_data_numpy):
    ...
    # the output dimension is the vector shape of the target vector
    # in this case, the shape is 1, because we are classifying a single value
    ...
    for t in range(epochs):
        ...
        loss = loss_fn(np.squeeze(y_pred), np.squeeze(y_train_tensor_Variable))
